...
def calculate_individual_power_spectra(
    signal,
    sfreq,
    window_length = 1,
    overlap = 0,
...
def plot_all_channel_power_spectra(
    channels, channel_names, title,
    sfreq, window_length, overlap, freq_range=(1, 100),
):
    """
...
# ---- 2.4 DEFINE PLOTTING FUNCTION ----

def plot_raw_vs_processed(raw_unprocessed, raw_processed, channel_picks, plot_title, start_time, duration,raw_title, processed_title):
    """
    Generates a two-subplot figure comparing raw and processed MEG data for a given set of channels.

    Args:
        raw_unprocessed (mne.io.Raw): The unprocessed raw data object.
        raw_processed (mne.io.Raw): The processed (e.g., filtered, HFC) raw data object.
        channel_picks (list): A list of channel names to include in the plot.
        plot_title (str): The main title for the entire figure.
        start_time (int, optional): The start time in seconds for the plot window.
        duration (int, optional): The duration in seconds for the plot window.
        raw_title (str): The title for the unprocessed data subplot.
        processed_title (str): The title for the processed data subplot.
    """
    # Create a figure with two subplots, stacked vertically
    fig, axes = plt.subplots(2, 1, figsize=(15, 10), sharex=True)
    
    # --- Plot 1: Unprocessed Data ---
    ...
    axes[0].set_xlabel('') # Remove x-label from the top plot
    ...
    fig.suptitle(plot_title, fontsize=16, y=1.0)
